 ...
 *     
 * 4．TPB速度の算出式変更
 *    以下のように変更し、素早いバトラーが連続行動しすぎないように制限した。
 *    ターンが回ってくるまでの速度反映式
 *         tpbSpeed = (10 + (param - 20) * 0.2).clamp(0, 15) + 1
 *    キャスト速度
 *         tpbCastSpeed = (10 + (param - 20) * 0.2).clamp(0, 15)
 *    スキル発動までに必要な時間
 *         castTime = (skillDelay) * 0.03
 * 
 * 5. TP上昇条件の変更
 ...
 * 
 * 11.スキル/アイテムのダメージ計算時、ガードなどを適用するかどうかを設定できるように拡張
 *   属性倍率、ダメージ倍率、回復率（リカバリーレート）、クリティカル倍率、ばらつき、ガード軽減の
 *   それぞれについて、ノートタグ指定で適用しないようにした。
 * 
 * 12.PDR,MDRの反映方法変更
 *   属性ベースで、
 *      物理属性によるダメージ分に対して PDR を適用
 *      魔法属性によるダメージ分に対して MDR を適用
 *   に変更した。
 *   物理属性、魔法属性はプラグインパラメータで設定する。
 *   スキル/アイテム属性が物理、魔法のいずれでもない場合、PDR,MDRの対象外になる。
 * 
 * 13.スキル、アイテムに物理ダメージ計算式、魔法ダメージ計算式を個別に設定できるように拡張
 *   ノートタグで指定しない場合、通常通りダメージ計算式が適用される。
 *   これまでのダメージ計算式は以下のように扱う。
 *     スキル命中タイプが 物理 ⇒ 物理ダメ計算式。但しノートタグで指定された物理ダメ計算式が優先される。
 *     スキル命中タイプが 魔法 ⇒ 魔法ダメ計算式。但しノートタグで指定された魔法ダメ計算式が優先される。
 *     スキル命中タイプが 必中 ⇒ その他計算式。
 * 
 * 14.HP/MP/TP数値色の変更
 *    HPは全快時、減ったとき、瀕死、死亡で個別に設定可能とした。
 *    MP/TPは全快時、減ったとき、死亡で個別に設定可能とした。
 *      
 * スキルの命中タイプにより、物理ダメージ計算式として使用するか、
 *   魔法ダメージ計算式とするかを分けるようにした。
 * 
 * ■ 使用時の注意
 * なし。
 * 
