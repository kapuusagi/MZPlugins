 ...
 * 
 * 強化成功率の計算
 *   (強化成功率) = ((a)基本強化成功率) * ((b)強化作業者の能力による成功率) * ((c)使用素材による成功率)
 * 
 *   但し最小強化成功率以下にはならない。
 * 
 * (a) 基本強化成功率
 ...
 *     強化難易度0(最低)で、1回目強化、作業者スキルレベルが10とすると、
 *     強化成功率補正は 1.0 - {0 + 0 - 10} * 0.05 = 1.5
 *     となり、成功率が50％アップする。
 *     強化難易度0(最低)で、10回目強化、作業者スキルレベルが10とすると、
 *     強化成功率補正は 1.0 - {0 + 10 - 10} * 0.05 = 1.0
 *     となり、成功率は変化ない。
 *     強化難易度10で、10回目強化、作業者スキルレベルが10とすると、
 *     強化成功率補正は 1.0 - {10 + 10 - 10} * 0.05 = 0.5
 *     となり、成功率は50%減少する。
 * (c) 使用素材による強化成功率
 *     (強化素材に設定された成功率)*(個数ボーナス)
 *     個数ボーナスは2個以上にしたとき、1個増える毎に50%上昇する。
 *     素材に設定された成功率が50%なら、2つ使用すると 50%×1.5 = 75% になる。
 * 
 * ■ 使用時の注意
 * 
 * ■ プラグイン開発者向け
